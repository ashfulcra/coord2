...
host/cwd-derived string (multi-session collision, hostname rot). The role's exclusive lease makes
...
- Installs from a git tag via `uv tool install` (proven v0.4.0 → v1.3.0).
...
- **Review lineage**: every PR dual-reviewed (Claude Opus adversarial + Codex independent), with
  fix→re-review loops; the upstream plan itself passed a Codex adversarial review (2026-07-04).
...
- **Live operator-loop result**: first day deployed, the asks fold surfaced a real blocker buried
  ...
  production data.
- **Incident-hardened**: catalog shape drift (fulcra-api 0.1.35) caused a duplicate-timeline
  incident in the predecessor; the postmortem produced dual-shape matchers, cache pinning, and the
