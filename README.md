...
| L2 | `fulcra-agent-tasks` | `task start/update/done` | typed status/priority/assignee lifecycle |
...
## Status
**`coord-engine` v0.1.0 + two skills built** — `fulcra-agent-reconcile` and `fulcra-agent-roles`.
**72 engine tests** + a live end-to-end run of reconcile/queries against the real Fulcra File Store.
Packaging decided (approach C) after independent + bus (Codex) review; open decisions resolved in doc
`03`. L2 (tasks), L5 (review), L6 (continuity), L7 (automation) not yet built.
